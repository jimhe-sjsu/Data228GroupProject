...
def build_predictions(db_path: Path = DUCKDB_PATH) -> int:
    """Build the serving DuckDB table. Returns the number of prediction rows."""
    ...
    with _connect(db_path) as con:
        ...
        row_count = con.execute(f"SELECT COUNT(*) FROM {SERVING_TABLE}").fetchone()[0]
    logger.info("Serving table built: %s rows", row_count)
    return row_count
