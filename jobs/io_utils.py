...
def create_spark_session(app_name="NYCTaxiCleaning", config=None):
    ...
    builder = (
        ...
        .config("spark.hadoop.mapreduce.output.fileoutputformat.compress.codec", "org.apache.hadoop.io.compress.SnappyCodec")
    )
...
def count_rows_from_parquet_metadata(spark, individual_files):
    """Read exact row counts from parquet file footers without scanning data."""
    jvm = spark._jvm
    conf = spark._jsc.hadoopConfiguration()
    parquet_file_reader = jvm.org.apache.parquet.hadoop.ParquetFileReader
    path_cls = jvm.org.apache.hadoop.fs.Path

    total = 0
    for file_path in individual_files:
        reader = parquet_file_reader.open(conf, path_cls(file_path))
        try:
...
def _expand_glob(spark, glob_path):
...
def read_parquet_from_hdfs(spark, paths, enforce_fn=None):
    ...
    providing a schema at read time does binary mapping rather than a cast.
    Reading one file at a time avoids both problems.
    """
    try:
        individual_files = []
        ...
        if not individual_files:
            raise RuntimeError("No parquet files found for paths: {}".format(paths))

        logger.info("Found {} parquet file(s) to read".format(len(individual_files)))
        ...
        raise RuntimeError(
            "Failed to read Parquet from HDFS. Paths: {}. Error: {}".format(paths, exc)
        ) from exc
...
def write_parquet_to_hdfs(df, path, partition_cols):
    try:
        ...
        raise RuntimeError(
            "Failed to write Parquet to HDFS at {}. Error: {}".format(path, exc)
        ) from exc
...
def write_to_mysql(df, config):
    ...
    try:
        ...
        logger.error("MySQL write failed: {}".format(exc))
        raise RuntimeError("Failed to write to MySQL: {}".format(exc)) from exc
...
def write_cleaning_report(step_log, raw_count, final_count, output_path):
    ...
    for idx, entry in enumerate(step_log, start=1):
        ...
        if b is not None and r is not None:
            pct_s = "{:.2f}%".format((r / b * 100)) if b > 0 else "0.00%"
            lines.append(
                "| {} | {} | {:,} | {:,} | {:,} | {} |".format(
                    idx, entry["step"], b, a, r, pct_s
                )
            )
        else:
            lines.append("| {} | {} | - | - | - | - |".format(idx, entry["step"]))
    ...
    lines += [
        ...
        "## Summary",
        "",
        "- **Raw rows in:** {:,}".format(raw_count),
        "- **Cleaned rows out:** {:,}".format(final_count),
        "- **Total rows removed:** {:,}".format(total_removed),
        "- **Overall reduction:** {:.2f}%".format(total_pct),
        "",
    ]
