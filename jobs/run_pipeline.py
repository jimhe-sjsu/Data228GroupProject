...
def main():
    ...
    try:
        ...
        before_distance_sample = df.select("trip_distance").sample(withReplacement=False, fraction=0.01, seed=42).toPandas()
        before_fare_sample = df.select("fare_amount").sample(withReplacement=False, fraction=0.01, seed=42).toPandas()
        ...
        after_distance_sample = df.select("trip_distance").sample(withReplacement=False, fraction=0.01, seed=42).toPandas()
        after_fare_sample = df.select("fare_amount").sample(withReplacement=False, fraction=0.01, seed=42).toPandas()
